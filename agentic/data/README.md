...
it. The kit (`agentic/skills/`, `templates/`, `workflows/`, `config/`, `scripts/`,
...
- `modules/`, `integrations/`, `shared-components/`, `tests/` — discovered/reconstructed context
  for brownfield work, one file per module (see `agentic/templates/module-context.yaml`).
...
with `agentic/templates/task.md`). Per-feature tasks produced by `task-breakdown-agent` go under
...
(`agentic/runtime/python/agentic_runtime/cli.py`) and hold no information that should be
committed. Safe to delete when nothing is midflight; the `SessionStart` hook checks
`runtime/state/active-task.json` and the run DB to detect anything left running.
