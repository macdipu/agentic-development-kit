...
└── runtime/
    ├── state/            gitignored   active-task pointer + runs/<run_id>.json store
...
Local runtime operating state: the active-task pointer, the run store (one JSON file per run under
`runtime/state/runs/`), and logs. Both are gitignored (see repo-root `.gitignore`) — they're
regenerated by running the runtime (`agentic/kit/runtime/python/agentic_runtime/cli.py`) and hold
no information that should be committed. Safe to delete when nothing is midflight; the
`SessionStart` hook checks `agentic/data/runtime/state/active-task.json` and the run store to
detect anything left running.
