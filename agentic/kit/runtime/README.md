...
The implementation path is `IMPLEMENTATION -> REVIEW -> QA -> RELEASE -> COMPLETED`. Planning is decided by evidence, not at `start`: leaving the decision stage (TECHNICAL; CONTEXT for `existing_task`) requires a `work-item-level-classifier` result with `outputs.classification`, `outputs.sprint_handling`, and `outputs.work_item_id`, and the rest of the route is rebuilt from it. PLANNING is added for `EPIC_STORY_TASK`/`STORY_TASK` and for `FULL_SPRINT_PLANNING`/`ADD_TO_EXISTING_SPRINT`/`BACKLOG_ONLY`; `BACKLOG_ONLY` ends after PLANNING; `TASK_ONLY`/`EXECUTE_EXISTING_TASK` with `NO_REPLAN`/`EXPEDITED` go straight to implementation. PLANNING cannot be left until the verdict's files exist under `agentic/data/project-context/`: `features/<id>/EPIC.md` (epic), `features/<id>/stories/STORY-*.md` (epic or story), `features/<id>/tasks/TASK-*.md` (any hierarchy but existing-task), and a `sprints/SPRINT-*.md` naming the id (sprint handling). `start --planning` overrides only the sprint handling. Preview/discovery routes do not plan.
