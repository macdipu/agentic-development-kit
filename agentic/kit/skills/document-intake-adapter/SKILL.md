...
Canonical work-item draft and source-to-field mapping, missing fields, conflicts, and referenced approvals requiring verification. Leave the source document at its existing path; record that path plus the assigned work-item ID so downstream skills can write derived artifacts to `agentic/data/project-context/features/<work-item-id>/` (see that folder's `README.md`).
